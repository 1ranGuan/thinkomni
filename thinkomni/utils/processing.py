...
from utils.gpt_parser import gpt_eval_score_retry as gpt_eval_score
...
def evaluate_prediction(prediction: str, answer: str, dataset: str, question: str = "") -> float:
    ...
    elif dataset in ["mathvista", "mathverse", "mathvision", "wemath", "chartqa", "logicvista"]:
        try:
            score = llm_eval_score(question, prediction, answer, dataset)
            # score = gpt_eval_score(question, prediction, answer, dataset)
        except:
            import time
            time.sleep(10)
            score = llm_eval_score(question, prediction, answer, dataset)
